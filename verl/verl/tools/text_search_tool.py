...
class TextSearchTool(BaseTool):
    ...
    def __init__(self, config: dict, tool_schema: OpenAIFunctionToolSchema):
        ...
        Config options:
            ...
            text_search_address: Address of text_search_server (e.g., "127.0.0.1:8000")
        ...
        # Keep fail-fast as default for backward compatibility.
        # Set text_search_fail_fast=false in tool config to continue training on search outages.
        self.text_search_fail_fast = config.get("text_search_fail_fast", True)
        # Optional per-instance hard limit for tool calls (None/0 means unlimited)
        self.max_text_search_calls = int(config.get("max_text_search_calls", 0) or 0)
        self.text_search_limit_reached_text = config.get(
            "text_search_limit_reached_text", "Text search call limit reached."
        )
    ...
    async def execute(
        ...
        # Increment and enforce per-instance call limit before launching retrieval.
        if instance_id in self._instance_dict:
            ...
            # Fallback in case execute() is called without a known instance.
            call_count = 1

        if self.max_text_search_calls > 0 and call_count > self.max_text_search_calls:
            return (
                ToolResponse(text=self.text_search_limit_reached_text),
                0.0,
                {"success": True, "query": query, "call_count": call_count, "limit_reached": True},
            )
        ...
        if not query or not isinstance(query, str):
            return (
                ...
                -0.05,
                {"success": False, "error": "invalid_query", "call_count": call_count},
            )
        ...
        if result is None:
            ...
            if self.text_search_fail_fast:
                raise RuntimeError(error_msg)
            return (
                ToolResponse(text=f"Error: {error_msg}"),
                0.0,
                {"success": False, "error": "search_failed", "call_count": call_count, "limit_reached": False},
            )
        ...
        return (
            ...
            0.0,
            {"success": True, "query": query, "call_count": call_count, "limit_reached": False},
        )
