...
class RLHFJSONDatasetV2(Dataset):
    ...
    def __init__(
        ...
        # Optional path prefix to resolve relative paths in meta.json / jsonl records.
        # This is OFF by default to avoid surprising behavior. Enable explicitly via:
        #   data.path_prefix=/abs/path/to/project_root
        # Useful for Ray multi-node where worker CWD may differ.
        self.path_prefix = config.get("path_prefix", None)
        if self.path_prefix:
            self.path_prefix = os.path.abspath(os.path.expanduser(str(self.path_prefix)))
        ...
        self.image_search_max_results = config.get("image_search_max_results", 2)
        ...
        if self.tool_config_path:
            try:
                ...
                logger.info(
                    f"Initialized tool schemas from {self.tool_config_path} for {'train' if self.is_train else 'val'} dataset"
                )
            except Exception as e:
        ...
        # Optional absolute-path prefix rewrite for migrated storage paths.
        self.path_rewrite_from = config.get("path_rewrite_from", "")
        self.path_rewrite_to = config.get("path_rewrite_to", "")

        self._read_files_and_tokenize()

    def _rewrite_legacy_abs_path(self, p: str):
        """Rewrite known legacy absolute path prefix when needed."""
        if p is None or not isinstance(p, str):
            return p

        def _rewrite_local_abs(local_path: str):
            if not os.path.isabs(local_path):
                return local_path
            old_prefix = self.path_rewrite_from
            new_prefix = self.path_rewrite_to
            if not old_prefix or not new_prefix:
                return local_path
            if not local_path.startswith(old_prefix):
                return local_path

            candidate = new_prefix + local_path[len(old_prefix):]
            # Prefer rewrite whenever the migrated path exists.
            if os.path.exists(candidate):
                return candidate
            # Fallback to old path if migrated path does not exist.
            return local_path

        if p.startswith("file://"):
            local = p[len("file://") :]
            rewritten_local = _rewrite_local_abs(local)
            if rewritten_local != local:
                return "file://" + rewritten_local
            return p

        return _rewrite_local_abs(p)

    def _resolve_media_path(self, p: str, root: str | None = None):
        """Resolve media path with optional root and legacy prefix rewrite support."""
        rewritten = self._rewrite_legacy_abs_path(p)
        if isinstance(rewritten, str) and os.path.isabs(rewritten):
            return rewritten
        if root:
            return os.path.join(root, str(rewritten).lstrip("./"))
        return self._maybe_prefix_path(rewritten)

    def _maybe_prefix_path(self, p: str):
        """If data.path_prefix is set, resolve relative paths against it; otherwise return as-is."""
        if not self.path_prefix:
            return self._rewrite_legacy_abs_path(p)
        if p is None:
            return p
        if not isinstance(p, str):
            return p
        p = self._rewrite_legacy_abs_path(p)
        ...
        if p.startswith("file://"):
            ...
            if os.path.isabs(local):
                return p
            local = local.lstrip("./")
            return "file://" + os.path.join(self.path_prefix, local)
        # Normal relative path
        rel = p.lstrip("./")
        return os.path.join(self.path_prefix, rel)
    ...
    def _read_files_and_tokenize(self, in_worker=False):
        ...
        for dataset_name, dataset_info in self.meta_info.items():
            # Load JSONL data
            annotation_path = self._maybe_prefix_path(dataset_info["annotation"])
            data_list = load_local_jsonl(annotation_path)
            ...
            root = self._maybe_prefix_path(root) if root else root
            for i, d in enumerate(data_list):
                ...
                if self.image_key in d:
                    ...
                    if not isinstance(images, list):
                        images = [images]
                    d[self.image_key] = [{"image": self._resolve_media_path(img, root=root)} for img in images]
                ...
                if self.video_key in d:
                    ...
                    if not isinstance(videos, list):
                        videos = [videos]
                    d[self.video_key] = [{"video": self._resolve_media_path(vid, root=root)} for vid in videos]
                ...
                if root and self.image_search_thumbnail_list_key in d:
                    ...
                    if isinstance(thumbnails, list):
                        d[self.image_search_thumbnail_list_key] = [
                            self._resolve_media_path(t, root=root) for t in thumbnails
                        ]
    ...
    def _extract_question_hint(self, messages: list) -> str:
        """Extract the latest user text as question hint for search summarization."""
    ...
    def _add_metadata_fields(self, row_dict: dict, item: int) -> dict:
        ...
        # Keep backward compatibility with legacy `image_search_summary`, but prefer
        # the new `image_search_results.organic[].section_texts` when available.
        legacy_summary = row_dict.pop(self.image_search_summary_key, None)
        section_summary_parts = []
        section_text_blocks = []
        image_search_results = row_dict.get("image_search_results", None)
        if isinstance(image_search_results, dict):
            organic_results = image_search_results.get("organic", [])
            if isinstance(organic_results, list):
                for organic_item in organic_results[: self.image_search_max_results]:
                    if not isinstance(organic_item, dict):
                        continue
                    current_block_parts = []
                    section_texts = organic_item.get("section_texts", None)
                    if isinstance(section_texts, list):
                        for section_text in section_texts:
                            if isinstance(section_text, str):
                                section_text = section_text.strip()
                                if section_text:
                                    section_summary_parts.append(section_text)
                                    current_block_parts.append(section_text)
                    elif isinstance(section_texts, str):
                        section_texts = section_texts.strip()
                        if section_texts:
                            section_summary_parts.append(section_texts)
                            current_block_parts.append(section_texts)
                    if current_block_parts:
                        section_text_blocks.append("\n\n".join(current_block_parts))

        row_dict["image_search_summary"] = (
            "\n\n".join(section_summary_parts) if section_summary_parts else legacy_summary
        )
        row_dict["image_search_text_blocks"] = section_text_blocks if section_text_blocks else None

        # Build tools_kwargs for AgentLoop
        tools_kwargs = row_dict.get("extra_info", {}).get("tools_kwargs", {})
        interaction_kwargs = row_dict.get("extra_info", {}).get("interaction_kwargs", {})
        need_tools_kwargs = row_dict.get("extra_info", {}).get("need_tools_kwargs", self.need_tools_kwargs)
        question_hint = self._extract_question_hint(row_dict.get("raw_prompt", []))
        image_path_for_image_search = None
        ...
        if isinstance(multi_modal_data, dict):
            ...
            if isinstance(image_paths, list) and image_paths:
                ...
                if isinstance(first_image, dict):
                    image_path_for_image_search = first_image.get("image", None)
                elif isinstance(first_image, str):
                    image_path_for_image_search = first_image

        # Add image_search_tool kwargs (key must match tool name from tool config).
        # Keep backward compatibility with title/thumbnail lists, and also support
        # text-only image search results (summary-only) for newer data format.
        if (
            row_dict.get("image_search_title_list")
            or row_dict.get("image_search_thumbnail_list")
            or row_dict.get("image_search_summary")
        ):
            tools_kwargs["image_search_tool"] = {
                "create_kwargs": {
                    ...
                    "image_search_text_blocks": row_dict.get("image_search_text_blocks"),
                    "question_hint": question_hint,
                    "image_path": image_path_for_image_search,
                }
            }
