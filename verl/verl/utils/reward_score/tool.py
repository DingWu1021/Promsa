...
def compute_score_llm(solution_str, ground_truth, raw_prompt, clients, llm_config, multi_modal_data=None, **kwargs):
    ...
    # Keep deterministic order, remove empty/duplicate candidates.
    ...
    for gt in ground_truth:
        if gt in (None, ""):
            continue
        gt_str = str(gt)
        if gt_str in seen_gt:
            continue
        seen_gt.add(gt_str)
        normalized_ground_truth.append(gt_str)
    ground_truth = normalized_ground_truth
    primary_ground_truth = ground_truth[0] if ground_truth else ""
    candidate_answers_str = ""
    if len(ground_truth) > 1:
        candidate_answers_str = "Candidate Answers:\n" + "\n".join(
            f"- {candidate}" for candidate in ground_truth
        ) + "\n"
...
10. If there are multiple candidate answers, mark the response as correct if it matches any one of them.
11. If a reference answer contains a date (year-level fact), accept answers within +/- 1 year of the reference.
12. If a reference answer is a single numeric value, apply a 10% tolerance window. For example, 10 cm should be judged correct for values in [9, 11], after unit normalization when possible.
13. If a reference answer is a numeric range, compare the predicted range and the reference range using interval IoU; judge as correct when IoU >= 50%.
14. For multiple choice questions (A, B, C, D), be more lenient. If the model provides the correct letter choice, even with additional text or formatting, consider it correct.
15. If the model's answer contains the correct choice letter (A, B, C, or D) anywhere in the response, and it's clear this is the intended answer, mark it as correct.
16. Ignore formatting issues like extra parentheses, brackets, or minor text variations as long as the core answer is correct.
...
7. For names, check for first and last name correctness. If the middle name is extra but correct, consider it correct.
8. For yes/no questions, the response must exactly match "Yes" or "No" to be correct.
9. If there are multiple candidate answers, mark the response as correct if it matches any one of them.
10. If a reference answer contains a date (year-level fact), accept answers within +/- 1 year of the reference.
11. If a reference answer is a single numeric value, apply a 10% tolerance window. For example, 10 cm should be judged correct for values in [9, 11], after unit normalization when possible.
12. If a reference answer is a numeric range, compare the predicted range and the reference range using interval IoU; judge as correct when IoU >= 50%.
13. For multiple choice questions (A, B, C, D), be more lenient. If the model provides the correct letter choice, even with additional text or formatting, consider it correct.
14. If the model's answer contains the correct choice letter (A, B, C, or D) anywhere in the response, and it's clear this is the intended answer, mark it as correct.
15. Ignore formatting issues like extra parentheses, brackets, or minor text variations as long as the core answer is correct.
...
<reason>Detailed reasoning following the evaluation principles.</reason>"""
    ...
    if answer is None:
        ...
        if em_check(answer, ground_truth):
            ...
            # Single judge request with all candidates embedded in one prompt.
            if has_candidate_answers:
                filled_prompt = prompt.format(
                    question=question,
                    ground_truth_answer=primary_ground_truth,
            ...
            llm_score = 1.0 if judge_decision == "Yes" else 0.0
...
def _has_model_caused_tool_error(
    data_non_tensor_batch: dict, penalize_tool_limit_reached: bool = False
) -> bool:
    """
    ...
    # Optional check: treat "tool call limit reached" as a format failure.
    if penalize_tool_limit_reached and tool_stats and any(s.get("limit_reached") for s in tool_stats):
        return True
...
def compute_format_score(solution_str: str, ground_truth, format_score, is_valid_format=True, **kwargs) -> float:
    ...
    - Optional: no tool call had limit_reached=True when penalize_tool_limit_reached=True
    ...
    Args:
        ...
        penalize_tool_limit_reached: If True, format_score=0 when any tool hits call-limit
    ...
    penalize_tool_limit_reached = kwargs.get("penalize_tool_limit_reached", False)
    if _has_model_caused_tool_error(
        data_non_tensor_batch, penalize_tool_limit_reached=penalize_tool_limit_reached
    ):
        return 0.0
