...
class MegatronPPOActor(BasePPOActor):
    ...
    def make_minibatch_iterator(self, data: DataProto) -> Iterable[DataProto]:
        ...
        non_tensor_select_keys = ["multi_modal_inputs"] if self.has_multi_modal_inputs else []
        if "__tool_calls__" in data.non_tensor_batch.keys():
            non_tensor_select_keys.append("__tool_calls__")
    ...
    def forward_backward_batch(
        ...
        if "__tool_calls__" in mini_batch.non_tensor_batch:
            mini_batch.batch["__tool_calls__"] = torch.as_tensor(
                mini_batch.non_tensor_batch["__tool_calls__"], dtype=torch.float32
            )
        ...
        def loss_func(output, data, meta_info):
            ...
            if not forward_only:
                ...
                policy_old_log_prob = old_log_prob
                # GSPO denominator shaping:
                # L -> L * (1 + c * H^alpha), where H is tool call count for this trajectory.
                if loss_mode == "gspo" and "__tool_calls__" in data:
                    c = float(self.config.policy_loss.get("tool_len_penalty_c", 0.0))
                    alpha = float(self.config.policy_loss.get("tool_len_penalty_alpha", 1.0))
                    if c > 0:
                        tool_calls = data["__tool_calls__"].to(device=log_prob.device, dtype=log_prob.dtype)
                        tool_calls = torch.clamp(tool_calls, min=0)
                        denom_scale = 1.0 + c * torch.pow(tool_calls, alpha)
                        # Equivalent transform:
                        # sum(logp-old)/(L*scale) == sum(logp-old_adj)/L
                        delta = (log_prob - old_log_prob) / denom_scale.unsqueeze(-1)
                        policy_old_log_prob = log_prob - delta
