...
class ToolRewardManager:
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.penalize_tool_limit_reached = kwargs.get("penalize_tool_limit_reached", False)
        # Optional penalty on tool-call count:
        # penalty = tool_call_penalty_weight * min(num_tool_calls / max_tool_calls, 1.0)
        self.tool_call_penalty_weight = float(kwargs.get("tool_call_penalty_weight", 0.5))
        self.tool_call_penalty_max_calls = int(kwargs.get("tool_call_penalty_max_calls", 7))
        ...
        if llm_judge_model:
            ...
            if is_azure:
                ...
                azure_endpoint = kwargs.get("llm_judge_azure_endpoint", "https://your-azure-endpoint.openai.azure.com/")
                ...
                if not llm_judge_urls:
                    raise ValueError(
                        ...
                        f"Example: +reward_model.reward_kwargs.llm_judge_urls=['127.0.0.1:8181']"
                    )
    ...
    def __call__(self, data: DataProto, return_dict: bool = False):
        ...
        def _process_item(i, data_item):
            ...
            try:
                ...
                # Merge canonical and alias answers so reward scoring can validate against all accepted answers.
                other_ground_truth = data_item.non_tensor_batch["reward_model"].get("other_ground_truth", [])
                if isinstance(ground_truth, list):
                    merged_ground_truth = list(ground_truth)
                elif ground_truth is None:
                    merged_ground_truth = []
                else:
                    merged_ground_truth = [ground_truth]

                if isinstance(other_ground_truth, list):
                    merged_ground_truth.extend(other_ground_truth)
                elif other_ground_truth not in (None, ""):
                    merged_ground_truth.append(other_ground_truth)

                # Deduplicate while preserving order and drop empty values.
                dedup_ground_truth = []
                seen_gt = set()
                for gt in merged_ground_truth:
                    if gt in (None, ""):
                        continue
                    gt_key = str(gt)
                    if gt_key in seen_gt:
                        continue
                    seen_gt.add(gt_key)
                    dedup_ground_truth.append(gt)
                merged_ground_truth = dedup_ground_truth
                ...
                common_kwargs = dict(
                    ...
                    penalize_tool_limit_reached=self.penalize_tool_limit_reached,
                )
                ...
                for name in reward_fn_names:
                    ...
                    s = compute_score_fns[name](valid_response_str, merged_ground_truth, **common_kwargs)
                    score[name] = s
                    total_score += s

                if len(reward_fn_names) > 0:
                    # Apply per-trajectory tool call count penalty:
                    # when num_tool_calls == max_calls, penalty reaches -tool_call_penalty_weight.
                    tool_stats = data_item.non_tensor_batch.get("tool_stats", [])
                    num_tool_calls = len(tool_stats) if tool_stats is not None else 0
                    if self.tool_call_penalty_max_calls > 0 and self.tool_call_penalty_weight > 0:
                        penalty_ratio = min(float(num_tool_calls) / float(self.tool_call_penalty_max_calls), 1.0)
                        tool_call_penalty = self.tool_call_penalty_weight * penalty_ratio
                    else:
                        tool_call_penalty = 0.0

                    score["num_tool_calls"] = num_tool_calls
                    score["tool_call_penalty"] = tool_call_penalty
                    score["score"] = total_score - tool_call_penalty
