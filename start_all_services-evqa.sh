...
ROOT_DIR="/path/to/workspace/ECCV/KBSearch-v4"
...
INDEX_PATH="${INDEX_PATH:-/path/to/workspace/search-agent/R1-Router/create_data_EVQA/text_kb/text_index_bge_m3.faiss}"
CORPUS_PATH="${CORPUS_PATH:-/path/to/workspace/search-agent/R1-Router/create_data_EVQA/text_kb/text_passages_bge_m3_meta.jsonl}"
KB_JSON_PATH="${KB_JSON_PATH:-/path/to/workspace/search-agent/R1-Router/create_data_EVQA/kb/encyclopedic_kb_wiki.json}"
...
echo "[all-services] starting local retrieval server on 8001 via search-r1-server-evqa.sh..."
(
  cd "${ROOT_DIR}"
  ...
  bash "${ROOT_DIR}/search-r1-server-evqa.sh"
