    ...
    async def generate_summary(self, query: str, content: str, model: str = 'gpt-3.5-turbo', prompt_type: str = 'gpt4o', content_limit: int = 100000, enable_thinking: bool = False) -> str:
        ...
        try:
            ...
            if prompt_type == 'mmsearch_r1':
                logger.info(f"[LLM] Using mmsearch_r1 prompt: 4-sentence summary style")
                system_prompt = """You are a helpful assistant. Your task is to summarize the main content of the given web page in no more than four sentences. Your summary should cover the overall key points of the page, not just parts related to the user's question.

If any part of the content is helpful for answering the user's question, prioritize it in the summary. Return only the final summary (max four sentences), with no reasoning process, analysis steps, or chain-of-thought."""
