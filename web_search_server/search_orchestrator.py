    ...
    async def process_query(self, parameters: str) -> Tuple[str, float, Dict[str, Any]]:
        ...
        async with self.search_semaphore:  # Limit concurrent searches
            try:
                ...
                # Step 4: Concatenate per-document summaries (skip final LLM summarization pass)
                successful_summaries = [s.strip() for s in summaries if isinstance(s, str) and s.strip()]
                total_urls = len(summaries)
                successful_urls = len(successful_summaries)

                if successful_urls == 0:
                    logger.error(f"[FINAL] ✗ No per-document summaries available for query: {_truncate_query(query)}")
                    return f"Error: Failed to generate final summary for query: {query}", 0.0, {}

                final_summary = "\n\n".join(successful_summaries)
                summary_type = "complete" if successful_urls == total_urls else "partial_mixed"
                completion_reason = (
                    "all_urls_successful" if successful_urls == total_urls
                    else f"{successful_urls}_of_{total_urls}_urls_successful"
                )
                result_metadata = {
                    "type": summary_type,
                ...
                logger.info(
                    f"[FINAL] Concatenated {successful_urls}/{total_urls} per-document summaries "
                    f"for query: {_truncate_query(query)}"
                )
