...
export TEXT_SEARCH_ADDRESS="127.0.0.1:8000"
export LOCAL_DATABASE_ADDRESS="127.0.0.1:8001"
export SUMMARIZER_BASE_URL="${SUMMARIZER_BASE_URL:-http://127.0.0.1:8123/v1}"
export SUMMARIZER_API_KEY="${SUMMARIZER_API_KEY:-EMPTY}"
# Not needed when judge runs against a local OpenAI-compatible server (LLM_JUDGE_URL).
export AZURE_OPENAI_API_KEY=""
export WANDB_API_KEY="${WANDB_API_KEY:-your_wandb_api_key_here}"
LLM_JUDGE_URL="127.0.0.1:8181"
# ==========================================================================
...
if [ "$NODE_RANK" -eq 0 ]; then
    ...
    # ==================== Training ====================
    MODEL_PATH="/path/to/workspace/0_download_model_bash/Qwen3-VL-8B-Instruct"
    ...
    # train_files=$(dirname $0)/train_deepeyes_4k_train_non_mcq.json
    # val_files=$(dirname $0)/test_infoseek.json
    train_files=/path/to/workspace/ECCV/KBSearch-v13/train_infoseek_val.json
    val_files=/path/to/workspace/ECCV/KBSearch-v13/test_infoseek_val_v1.json

    model_name=$(basename "$MODEL_PATH")
    # Fixed experiment name for checkpoint matching (no timestamp)
    exp_name=ProMSA_${model_name}_lr1e-5
    ...
    python3 -m verl.trainer.main_ppo \
        ...
        +actor_rollout_ref.actor.policy_loss.tool_len_penalty_c=0.04 \
        +actor_rollout_ref.actor.policy_loss.tool_len_penalty_alpha=1.0 \
        actor_rollout_ref.actor.loss_agg_mode=seq-mean-token-mean \
        algorithm.use_kl_in_reward=False \
        data.custom_cls.path="pkg://verl.utils.dataset.rl_dataset_json_v2" \
        data.custom_cls.name="RLHFJSONDatasetV2" \
        +data.path_prefix="/path/to/workspace/ECCV/KBSearch-v13" \
        ...
        +data.image_search_max_results=4 \
        ...
        +actor_rollout_ref.rollout.multi_turn.directly_provide_image_search=False \
        ...
        +reward_model.reward_kwargs.penalize_tool_limit_reached=True \
        +reward_model.reward_kwargs.tool_call_penalty_weight=0.5 \
        +reward_model.reward_kwargs.tool_call_penalty_max_calls=7 \
        +reward_model.reward_kwargs.llm_judge_model='Qwen3-VL-8B-Instruct' \
        ...
        +reward_model.val_reward_kwargs.penalize_tool_limit_reached=True \
        +reward_model.val_reward_kwargs.llm_judge_model='Qwen-vl-8b' \
        +reward_model.val_reward_kwargs.llm_judge_urls=["${LLM_JUDGE_URL}"] \
        +reward_model.val_reward_kwargs.llm_judge_api_key="token-abc123" \
        ...
        trainer.project_name='ProMSA_rl' \
        trainer.experiment_name=$exp_name \
        trainer.n_gpus_per_node=8 \
        trainer.nnodes=$num_nodes \
        trainer.save_freq=10 \
        trainer.test_freq=-1 \
        trainer.rollout_data_dir="/path/to/workspace/ECCV/KBSearch-v13/rollout_data_infoseek_tn-gspo-new-last/$exp_name/train" \
        trainer.validation_data_dir="/path/to/workspace/ECCV/KBSearch-v13/rollout_data_infoseek_tn-gspo-new-last/$exp_name/validation" \
        trainer.default_local_dir="/path/to/workspace/ECCV/KBSearch-v13/checkpoints/ProMSA_rl-tn-gspo-new-last/$exp_name" \
